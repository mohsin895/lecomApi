...
class CouponCodeController extends Controller
{
    public function updateCouponCode(Request $request)
    {
       DB::beginTransaction();
        try{
            ...
            if(!empty($dataInfo)){
                $staff=Staff::where('id',Auth::guard('staff-api')->user()->id)
                ->first();
                $dataInfo->isdiscount_in_percent=$request->couponType;
                $dataInfo->staff_id = isset($staff) ? $staff->id : null;
                ...
                $staff=Staff::where('id',Auth::guard('staff-api')->user()->id)
                    ...
                    if($dataInfo->save()){
                        ...
                        $responseData=[
                                        'errMsgFlag'=>false,
                                        'msgFlag'=>true,
                                        'msg'=>'Coupon Code Information Added Successfully.',
                                        'errMsg'=>null,
                                    ];
    ...
    {
    	$query=VoucherDiscount::whereNull('staff_id')->whereNull('deleted_at')->orderBy('id','desc');
    ...
    public function getCouponCodeListStaff(Request $request)
    {
    	$query=VoucherDiscount::whereNull('seller_id')->whereNull('deleted_at')->orderBy('id','desc');
